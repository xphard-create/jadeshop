...
class AdminPanel {
  constructor() {
    ...
    // ID del comerciante (temporal - hardcodeado)
    this.comercianteId = 2;
  ...
  // Agregar comerciante_id a las URLs
  getApiUrl(endpoint) {
    const separator = endpoint.includes("?") ? "&" : "?";
    return `${endpoint}${separator}comerciante_id=${this.comercianteId}`;
  }
  ...
  async loadDashboard() {
    try {
      const response = await fetch(this.getApiUrl("/api/admin/dashboard"));
