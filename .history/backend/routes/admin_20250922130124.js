...
router.post("/pedidos", requireAuth, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO pedidos (comerciante_id, codigo_pedido, cliente_telefono, total, estado, cliente_nombre, cliente_email, cliente_whatsapp, created_at, updated_at)
   VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW()) RETURNING id`,
      [
        comerciante_id,
        codigoPedido,
        cliente_whatsapp || "", // Usar el WhatsApp como teléfono por defecto
        JSON.stringify(productos), // Agregar productos como JSON
        total,
        estado,
        cliente_nombre,
        cliente_email,
