...
const getComercianteId = (req, res, next) => {
  // Temporal: obtener comerciante_id del query parameter
  const comerciante_id = req.query.comerciante_id || req.body.comerciante_id;

  if (!comerciante_id) {
    return res.status(400).json({
      error: "comerciante_id requerido",
      message: "Agrega ?comerciante_id=X a la URL",
    });
  }

  req.comerciante_id = parseInt(comerciante_id);
  next();
};
...
router.get("/dashboard", getComercianteId, async (req, res) => {
  try {
...
router.get("/productos", getComercianteId, async (req, res) => {
  try {
    const { comerciante_id } = req;
...
router.post("/productos", getComercianteId, async (req, res) => {
  try {
    const { comerciante_id } = req;
...
router.put("/productos/:id", getComercianteId, async (req, res) => {
  try {
    const { id } = req.params;
    const { comerciante_id } = req;
    const { nombre, descripcion, precio, stock, categoria, imagen_url } =
...
router.delete("/productos/:id", getComercianteId, async (req, res) => {
  try {
    const { id } = req.params;
    const { comerciante_id } = req;
